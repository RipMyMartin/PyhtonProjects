...
class Zoo:
    ...
    def can_add_animal(self, animal: Animal) -> bool:
        ...
        for a in self.animals:
            if a is animal or (a.name == animal.name and a.specie == animal.specie):
                return False
...
if __name__ == '__main__':
    ...
    animal1 = Animal("Dog", "Canine", 3)
    animal2 = Animal("Cat", "Feline", 5)
    animal3 = Animal("Moneky", "Makaka", 12)

    my_zoo = Zoo("My Zoo", 10)
    ...
    all_animals = my_zoo.get_all_animals()
    print("All animals in the zoo:")
    for animal in all_animals:
        print(animal.name)

    animals_by_age = my_zoo.get_animals_by_age()
    print("\nAnimals sorted by age:")
    for animal in animals_by_age:
        print(f"{animal.name} - Age: {animal.age}")

    animals_sorted_alphabetically = my_zoo.get_animals_sorted_alphabetically()
    print("\nAnimals sorted alphabetically:")
    for animal in animals_sorted_alphabetically:
        print(animal.name)
